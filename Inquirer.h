    ...
    while (1) {
        ...
        if (ch == 0 || ch == KEY_ARROWS) {
            ...
            if (cur > 0) {
                ...
                for (size_t j = cur; j < len + 1; j++) putch('\b');
            }
        }
        else if (isprint(ch)) {
            if (len < out_size - 1) {
                ...
                cur++;
                for (size_t j = cur; j < len; j++) putch('\b');
            }
        }
    }
...
char* TextEx(const char* message, TextParams* p) {
    ...
    if (p) {
        if (p->amark)           params.amark = p->amark;
        if (p->qmark)           params.qmark = p->qmark;
        if (p->instruction)     params.instruction = p->instruction;
        if (p->validation)      params.validation = p->validation;
        if (p->invalid_message) params.invalid_message = p->invalid_message;
        if (p->data)            params.data = p->data;

        if (p->flags)            params.flags |= p->flags;
    }
    ...
    if (strcmp(params.instruction, "") != 0)
        printf(C_YELLOW"%s "C_RESET"%s "C_BWHITE"%s "C_BBLUE, params.qmark, message, params.instruction);
    else
        printf(C_YELLOW"%s "C_RESET"%s "C_BBLUE, params.qmark, message);
    ...
    while (1) {
        ...
        if (ch == 0 || ch == KEY_ARROWS) {
            int ext = _getch();
            if (ext == KEY_ARROW_L) {
                if (cur > 0) { cur--; putch('\b'); }
            }
            else if (ext == KEY_ARROW_R) {
                if (cur < len) { putch(out[cur]); cur++; }
            }
        }
        else if (ch == KEY_CTRL_C) {
            printf(C_RESET);
            ...
            printf(BOTTOM_LEFT C_BWHITE BG_RED C_BOLD"%s"C_RESET, params.invalid_message);
            printf("\x1b[u");
        }
        else if (ch == KEY_BACKSPACE) {
            if (cur > 0) {
